...
///
/// https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/move-examples/groth16_example/sources/groth16.move
...
module groth16_example::groth16 {
    ...
    /// Verifies a BN254 Groth16 proof where all inputs are already serialized for Move (`vector<u8>`).
    /// - G1 and G2 points are expected to be uncompressed.
    /// - Fr inputs are expected in little-endian serialized form.
    public fun verify_bn254_proof_from_bytes(
        vk_alpha_g1: vector<u8>,
        vk_beta_g2: vector<u8>,
        vk_gamma_g2: vector<u8>,
        vk_delta_g2: vector<u8>,
        vk_gamma_abc_g1: vector<vector<u8>>,
        public_inputs: vector<vector<u8>>,
        proof_a: vector<u8>,
        proof_b: vector<u8>,
        proof_c: vector<u8>,
    ): bool {
        let vk_alpha = std::option::extract(&mut deserialize_element<Bn254G1, Bn254FormatG1Uncompr>(&vk_alpha_g1));
        let vk_beta = std::option::extract(&mut deserialize_element<Bn254G2, Bn254FormatG2Uncompr>(&vk_beta_g2));
        let vk_gamma = std::option::extract(&mut deserialize_element<Bn254G2, Bn254FormatG2Uncompr>(&vk_gamma_g2));
        let vk_delta = std::option::extract(&mut deserialize_element<Bn254G2, Bn254FormatG2Uncompr>(&vk_delta_g2));

        let proof_a = std::option::extract(&mut deserialize_element<Bn254G1, Bn254FormatG1Uncompr>(&proof_a));
        let proof_b = std::option::extract(&mut deserialize_element<Bn254G2, Bn254FormatG2Uncompr>(&proof_b));
        let proof_c = std::option::extract(&mut deserialize_element<Bn254G1, Bn254FormatG1Uncompr>(&proof_c));

        let vk_uvw_gamma_g1 = vector[];
        let i = 0;
        while (i < std::vector::length(&vk_gamma_abc_g1)) {
            std::vector::push_back(
                &mut vk_uvw_gamma_g1,
                std::option::extract(&mut deserialize_element<Bn254G1, Bn254FormatG1Uncompr>(std::vector::borrow(&vk_gamma_abc_g1, i))),
            );
            i = i + 1;
        };

        let public_inputs_elements = vector[];
        let j = 0;
        while (j < std::vector::length(&public_inputs)) {
            std::vector::push_back(
                &mut public_inputs_elements,
                std::option::extract(&mut deserialize_element<Bn254Fr, Bn254FormatFrLsb>(std::vector::borrow(&public_inputs, j))),
            );
            j = j + 1;
        };

        verify_proof<Bn254G1, Bn254G2, Bn254Gt, Bn254Fr>(
            &vk_alpha,
            &vk_beta,
            &vk_gamma,
            &vk_delta,
    ...
    /// On-chain entry point equivalent of `verify_bn254_proof_from_bytes`.
    /// Aborts if verification fails.
    public fun verify_bn254_proof_from_bytes_entry(
        _signer: &signer,
        vk_alpha_g1: vector<u8>,
        vk_beta_g2: vector<u8>,
        vk_gamma_g2: vector<u8>,
        ...
        assert!(verify_bn254_proof_from_bytes(
            vk_alpha_g1,
            vk_beta_g2,
            vk_gamma_g2,
            vk_delta_g2,
    ...
    #[test]
    fun test_verify_proof_with_bls12381() {
        let fx = &account::create_signer_for_test(@0x1);
        enable_cryptography_algebra_natives(fx);
    ...
    #[test]
    fun test_verify_proof_prepared_with_bls12381() {
        let fx = &account::create_signer_for_test(@0x1);
        enable_cryptography_algebra_natives(fx);
    ...
    #[test]
    fun test_verify_proof_prepared_fq12_with_bls12381() {
        let fx = &account::create_signer_for_test(@0x1);
        enable_cryptography_algebra_natives(fx);
